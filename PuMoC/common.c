...
#include "common.h"
extern int isdebug;
extern char* filenametodisplay;
#define MAX_RESOURCE_MARKS 7
...
void *common_malloc (size_t bytes)
{
  ...
  if((memory_threshold/(1024*1024))>2048)
  {
  	printf("\nmemory out 2GB\n");
  	exit(0);   	
  }
	...
	{
		//printf("%lu %lu aaaaaaaaaaaaaaaaaa malloc \n ",memory_current,mark_memory_max[i]);
...
void common_resource_setmark (int res_mark_nr)
{
	...
	//printf("%l %l \n****************************** ",memory_current,memory_threshold );
#endif
...
void common_resource_displaymark (int res_mark_nr, char *comment, float *tp)
{
	...
	{
	long  memory;
	//printf( "  - Memory : %lu start  %lu  max u bytes\n\n", mark_memory_start[res_mark_nr],mark_memory_max[res_mark_nr] );
	memory = mark_memory_max[res_mark_nr] - mark_memory_start[res_mark_nr];
	printf( "  - Memory : %lu bytes,  %lu Kbytes,  %.2f Mbytes\n\n", memory,memory/1024, (float)memory/(1024*1024));
...
void common_resource_patch (int res_mark_nr, char *comment, float *tp, int Size)
{
	float time;

  FILE* fptr=fopen("patch.txt", "a");
  fprintf(fptr, "%d          ", Size); 
	if (!mc_stats) return;
	assert((res_mark_nr >= 0) && (res_mark_nr < MAX_RESOURCE_MARKS));
	...
	//printf( "  - Memory : %lu start  %lu  max u bytes\n\n", mark_memory_start[res_mark_nr],mark_memory_max[res_mark_nr] );
	memory = mark_memory_max[res_mark_nr] - mark_memory_start[res_mark_nr];
	fprintf(fptr, "      %.2f Mbytes  ", (float)memory/(1024*1024) );
	mark_memory_start[res_mark_nr] = -1;
	mark_memory_max[res_mark_nr] = -1;
	}
#endif
