...
int binding_vars=0;                  
int step_length=0;                   
DdManager *CTLBDDMgr=NULL;
DdNode **bddValVar=NULL;
DdNode *CTLFullset =NULL;
DdNode *CTLEmptyset =NULL;

// end: added by SONG Fu
...
int compute_step_length(int num)
{
	if(num==0) return 0;
	return num==1? 1: ceil(log10(num)/log10(2));
}
void init_CTL_BDD_Manager()
{
	int i;
	step_length = compute_step_length(abpdsValNum);
	bddValVar = malloc( ( step_length * ctlQuantVarNum) * sizeof(DdNode*));
	CTLBDDMgr = Cudd_Init(step_length*ctlQuantVarNum, 0, CUDD_UNIQUE_SLOTS,CUDD_CACHE_SLOTS,0);
	Cudd_AutodynDisable(CTLBDDMgr);
	
	for (i = 0; i < step_length*ctlQuantVarNum; i++)
		bddValVar[i] = Cudd_bddIthVar(CTLBDDMgr,i);

	CTLFullset = Cudd_ReadOne(CTLBDDMgr);
	CTLEmptyset = Cudd_Not(CTLFullset);	
