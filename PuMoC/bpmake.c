...
void bp_create_sequence (bp_fun_t,bp_stmt_t,Name,Name);
...
Name bp_create_label (bp_fun_t fun, bp_stmt_t stmt, Name label)
{
	char tmp[256];	/* fixme */
	Name ret;

	if (stmt && stmt->label > 0)
		sprintf(tmp,"%s:%s",name_cstring(fun->funname),
				    name_cstring(stmt->label));
	else if (stmt && stmt->label < 0)
		sprintf(tmp,"%s/%d",name_cstring(fun->funname),-(stmt->label));
	else if (label)
		sprintf(tmp,"%s:%s",name_cstring(fun->funname),
				    name_cstring(label));
	else
		sprintf(tmp,"%s/%d",name_cstring(fun->funname),++bp_labelcount);
...
DdNode* bp_get_var (bp_idref_t idref)
{
	...
	if (ident->fun) return (idref->primed? yvars : ypvars)[ident->index];
	return (idref->primed? pvars : ppvars)[ident->index];
}
...
DdNode* bp_add_clause (DdNode *base, DdNode *add)
{
	...
	Cudd_Ref(tmp0 = Cudd_bddAnd(mc_mgr,base,add));
	Cudd_RecursiveDeref(mc_mgr,base);
	return tmp0;
}
	...
	for (i = size - 1; i >= 0; i--)
	{
		switch (flag[i])
		{
		  ...
		  case BP_HAVE_EXPR:
			...
			Cudd_RecursiveDeref(mc_mgr,expr[i]);
			break;
		  case BP_EQUIV:
			j = perm[i];
			if (j < global_vars)
...
void bp_add_equivs (bp_fun_t fun, char *live, int *gxlat, int *lxlat)
{
	...
	if (lxlat) for (var = fun->locals; var; var = var->next)
	{
		...
		else if (bp_flag[aindex] == BP_HAVE_EXPR)
		{
			...
			Cudd_RecursiveDeref(mc_mgr,bp_expr[aindex]);
		}
	}
}
...
DdNode* bp_make_expr (bp_expr_t expr)
{
	...
	switch (expr->token)
	{
	    ...
	    case BP_IDENT:
			...
			return tmp0;
	    case BP_CONST:
			Cudd_Ref(expr->left? fullset : emptyset);
			return expr->left? fullset : emptyset;
	    default:
			common_error("can't happen");
	  		return NULL;
	}
...
DdNode* bp_make_choose_expr (DdNode *var, bp_expr_t expr)
{
	...
	if (expr->token == BP_CHOOSE)
	{
		...
		Cudd_Ref(tmp3 = Cudd_bddAnd(mc_mgr,Cudd_Not(tmp0),Cudd_Not(tmp1)));

		Cudd_Ref(tmp2 = Cudd_bddAnd(mc_mgr,Cudd_Not(tmp0),tmp1));
		Cudd_RecursiveDeref(mc_mgr,tmp1);
		Cudd_Ref(tmp1 = Cudd_bddAnd(mc_mgr,tmp2,Cudd_Not(var)));
		Cudd_RecursiveDeref(mc_mgr,tmp2);
		Cudd_Ref(tmp2 = Cudd_bddOr(mc_mgr,tmp1,tmp3));
		Cudd_RecursiveDeref(mc_mgr,tmp1);
		Cudd_RecursiveDeref(mc_mgr,tmp3);

		Cudd_Ref(tmp1 = Cudd_bddAnd(mc_mgr,var,tmp0));
		Cudd_RecursiveDeref(mc_mgr,tmp0);
		Cudd_Ref(tmp0 = Cudd_bddOr(mc_mgr,tmp1,tmp2));
	...
	Cudd_Ref(tmp1 = Cudd_bddXnor(mc_mgr,var,tmp0));
	Cudd_RecursiveDeref(mc_mgr,tmp0);
...
void bp_create_assign (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
		ABPDSInsertTransition(&mc_abpds, bp_qstate, ctlTree, label1, bp_qstate, label2, 0);
		return;
	}
	DdNode *tmp1, *tmp2;
	bp_idref_t ref;
	...
	Cudd_Ref(tmp2 = fullset);
	tmp2 = bpa_build_expr(fun,tmp2);

	pds_insert_trans(&mc_pds,
		bp_qstate,label1,bp_qstate,label2,0,tmp2,NULL, bp_prio(stmt));
...
void bp_create_call (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{	
		
	Name retlabel, savelabel, restorelabel;
	...
	if(mc_logic_lang_CTL)
	{
		Name ll=bp_create_label(target,target->stmt,0);
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
		ABPDSInsert2StackList(&(mc_abpds.stackList), ll);	
		ABPDSInsertTransition(&mc_abpds, bp_qstate, ctlTree, label1, bp_qstate, ll, label2);
		return;
	}
		
	if (call_crosses_comp(fun,target))
	{
		do_save_in_call = !mc_restrict && !stepwise(target->num_args);
...
void bp_create_assert (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
	...
	pds_insert_trans(&mc_pds,
		bp_qstate,label1,bp_qstate,label2,0,tmp0,NULL,bp_prio(stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp0);
}

/***************************************************************************/

void bp_create_constrain(bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
...
void bp_create_if (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
...
void bp_create_while (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	bp_stmt_t tmp = stmt->next;
	bp_expr_t expr = stmt->e.c.expr;
...
void bp_create_return (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
		ABPDSInsertTransition(&mc_abpds, bp_qstate, ctlTree, label1, bp_qstate, 0, 0);
		return;
	}	
...
void bp_create_skip (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
...
void bp_create_goto (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	Name target = bp_create_label(fun,NULL,stmt->e.a.label);

	if(mc_logic_lang_CTL)
	{
		ABPDSInsert2StackList(&(mc_abpds.stackList), label1);
		ABPDSInsert2StackList(&(mc_abpds.stackList), target);
		ABPDSInsertTransition(&mc_abpds, bp_qstate, ctlTree, label1, bp_qstate, target, 0);
		if(stmt->e.a.flag==12341234)
		{
			if(stmt->e.a.label2!=-1)
			{
				Name target2=  bp_create_label(fun,NULL,stmt->e.a.label2);
				ABPDSInsert2StackList(&(mc_abpds.stackList), target2);
				ABPDSInsertTransition(&mc_abpds, bp_qstate, ctlTree, label1, bp_qstate, target2, 0);			
			}
		}
		return;
	}	
	...
	pds_insert_trans(&mc_pds,
		bp_qstate,label1,bp_qstate,target,0,tmp0,NULL,bp_prio(stmt));
...
void bp_create_sequence (bp_fun_t fun, bp_stmt_t stmt, Name label1, Name label2)
{
	void (*cfun)(bp_fun_t,bp_stmt_t,Name,Name) = NULL;
	Name nextlabel;
	...
	while (stmt)
	{
	    switch (stmt->type)
	    {
			...
			default:		   common_error("can't happen");
	    }
...
void bp_create_fun (bp_fun_t fun)
{
	...
	if (!fun->funnode->dfsnum)
	{
		if (mc_verbose > 0) 
			printf("skipping unreachable function %s\n",
					name_cstring(fun->funname));
		return;			
	}
  ...
  else
  {
  	...
  	if (fun->enforce)
  	{
  		if (fun->funname == name_create("main"))
  		{
  			bp_prime_enforce(fun->enforce,0);
  			Cudd_RecursiveDeref(mc_mgr,bp_main_enforce);
  	...
  	else	Cudd_Ref(bp_enforce_expr = fullset);
  
  	/* set equality for all vars */
  	memset(bp_flag,BP_EQUIV,bpa_size);
  	Cudd_Ref(bp_skip_expr = fullset);
  	...
  	Cudd_RecursiveDeref(mc_mgr,bp_skip_expr);
  	if (fun->enforce) Cudd_RecursiveDeref(mc_mgr,bp_enforce_expr);
  }
}
...
void bp_create_pds ()
{
  ...
  {	
  	...
  	if(!mc_satabs) bp_build_call_graph();
  /*	if (mc_elimdead == 2 || mc_queue == 2) bp_build_flow_graph();
  	if (mc_elimdead == 1) bp_elim_dead_simple();
  	if (mc_elimdead == 2) bp_mod_ref();  */
  
  	bp_qstate = name_create("-q-");
    ABPDSInsert2StateList(&(mc_abpds.ctrlList), bp_qstate);
  	mc_abpds.startState = bp_qstate;

  	mc_abpds.startStack = name_stack_empty();
  	fun = (bp_fun_t)bp_lookup(bp_funtree,name_create("main"),-1);
  	if (!fun) common_error("main function does not exist");
  	
  	Name ll=bp_create_label(fun,fun->stmt,0);
  	name_stack_push(&mc_abpds.startStack,sharp,NULL);	
  	name_stack_push(&mc_abpds.startStack, ll , NULL);
    ABPDSInsert2StackList(&(mc_abpds.stackList), ll);
  	...
  	if (mc_verbose > 2)
  		printf("start at: %s %s\n",name_cstring(bp_qstate),
  			name_cstring(mc_abpds.startStack->name));
  }
	else
  {
  	bp_fun_t fun;
  	...
  	if (mc_elimdead == 1) bp_elim_dead_simple();
  	if (mc_elimdead == 2) bp_mod_ref();
  	if (mc_queue == 2 && mc_poststar) bp_compute_distances();
  
  	bp_labeltree = NULL;
  	global_vars = bp_maxrets;
  	local_vars = bp_maxlocs;
  	...
  	for (i = 0; i < bp_maxrets; i++)
  	    Cudd_Ref(bp_gequal[i] = Cudd_bddXnor(mc_mgr,pvars[i],ppvars[i]));
  	for (i = 0; i < bp_maxlocs; i++)
  	    Cudd_Ref(bp_lequal[i] = Cudd_bddXnor(mc_mgr,yvars[i],ypvars[i]));
  	for (i = 0; i < bp_maxlocs; i++)
  	    Cudd_Ref(bp_l2equal[i] = Cudd_bddXnor(mc_mgr,ypvars[i],yppvars[i]));
  
  	bp_qstate = name_create("-q-");
  	Cudd_Ref(bp_main_enforce = fullset);
  
  	for (fun = bp_functions; fun; fun = fun->next)
  		bp_create_fun(fun);
  
  	mc_pds.start_state = bp_qstate;
  	mc_pds.start_bdd = bp_main_enforce;
  	mc_pds.start_stack = name_stack_empty();
  	fun = (bp_fun_t)bp_lookup(bp_funtree,name_create("main"),-1);
  	if (!fun) common_error("main function does not exist");
  	name_stack_push(&mc_pds.start_stack,
  		bp_create_label(fun,fun->stmt,0), fullset);
  	if (mc_verbose > 2)
  		printf("start at: %s %s\n",name_cstring(bp_qstate),
  			name_cstring(mc_pds.start_stack->name));
  }
}
