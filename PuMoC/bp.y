...
%type <stmt> iteration_statement jump_statement dead_statement
...
function_definition
	: fun_type IDENTIFIER parms BEG decl_list enforce statement_list END
	  {
		int i;

		$$ = alloc_bp_fun();
		$$->funname = name_create($2);
		$$->num_returns = $1;
		$$->num_args = $3;
	  ...
	  |  fun_type IDENTIFIER parms BEG END 
	    {  $$=NULL;  }
	  ;
...
label_list
		: label_list IDENTIFIER ':'
		  {
	  			$$ = common_malloc(sizeof(struct ABPDSStackListStruct));
					$$->stack = name_create($2);
					$$->next =$1;
		  }
		| 
...
parallel_assign 
  ...
  | assign_id_list ASSIGN call
	...
	| assign_id_list ASSIGN expression_list CONSTRAIN '(' expression ')'
	  {
	  bp_idref_t tmp1, tmp2, tmp3;
...
asgn_id	: IDENTIFIER			
	  {
		$$ = alloc_bp_idref();
		$$->ident = bp_find_decl(name_create($1),bp_funcount);
		$$->primed = 1;
		$$->next = NULL;
	  } ;
...
jump_statement
	...
	| GOTO IDENTIFIER
	  {
		new_bp_stmt($$,BP_GOTO);
		$$->e.a.label = name_create($2);
	  } 
	| GOTO IDENTIFIER  ',' IDENTIFIER
	  {
		new_bp_stmt($$,BP_GOTO);
		$$->e.a.label = name_create($2);
		$$->e.a.label2=name_create($4);
		$$->e.a.flag=12341234;
	  }  ;


dead_statement : DEAD assign_id_list
   {
	   new_bp_stmt($$,BP_SKIP);
	 };
/***** expressions *****/
	...
	: IDENTIFIER 	
	  { 
		...
		else
		{
			...
			ref->ident =
			   bp_find_var(bp_nametree,name_create($1),bp_funcount);
			ref->primed = 0;
	...
	| '\'' IDENTIFIER
	  {
		bp_idref_t ref = alloc_bp_idref();
		ref->ident =
			bp_find_var(bp_nametree,name_create($2),bp_funcount);
		ref->primed = 1;
		new_bp_expr($$,BP_IDENT,(bp_expr_t)ref,NULL);
	  }
	| INTEGER 
