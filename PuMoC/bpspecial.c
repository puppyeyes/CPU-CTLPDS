...
void bp_call_extra (bp_fun_t fun, bp_stmt_t stmt, bp_fun_t target,
			Name fromstate, Name restorelabel)
{
	DdNode *tmp0;
	bp_ident_t var;
	...
	for (var = target->returns; var; var = var->next)
	{
	    ...
	    Cudd_Ref(tmp1 = Cudd_bddXnor
			    (mc_mgr,pvars[var->index],ppvars[var->index]));
	    tmp0 = bp_add_clause(tmp0,tmp1);
	    Cudd_RecursiveDeref(mc_mgr,tmp1);
	}

	pds_insert_trans(&mc_pds,fromstate,restorelabel,
			bp_qstate,restorelabel,0,tmp0,NULL,bp_prio(stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp0);
}

Name bp_tmpstate(bp_fun_t fun, bp_fun_t target)
{
	Name tmpstate;
	static int count = 0;
	char *s = malloc(strlen(name_cstring(fun->funname))
			+ strlen(name_cstring(target->funname)) + 10);
	sprintf(s,"%s-%s-%d",name_cstring(fun->funname),
			     name_cstring(target->funname),++count);
	tmpstate = name_create(s);
	free(s);
	return tmpstate;
}
...
void bp_special_call (bp_fun_t fun, bp_stmt_t stmt, bp_fun_t target,
						Name label1, Name retlabel)
{
	DdNode *tmp1, *tmp2;
	Name label2, tmplabel;
	Name targ1 = 0, targ2;
	...
	if (mc_verbose > 2) printf("special_call %d\n",target->num_args);
	...
	while (base < target->num_args)
	{
		...
		{
			if (!ref) break;
			tmp1 = bp_make_choose_expr(yvars[fml->index],ref->expr);
				...
				var = var->next, cb = cb->next)
		{
			...
			if (occbyfmls[var->index] == base)
			{
				/* if the var has just been overwritten,
				 * shift it into the globals */
				Cudd_Ref(tmp1 = Cudd_bddXnor
				  (mc_mgr,pvars[cb->index],ypvars[var->index]));
		 ...
		 * into locals earlier */
		if (!mc_restrict && call_crosses_comp(fun,target))
		{
		...
		if (!base && !mc_restrict)
			for (var = fun->locals; var; var = var->next)
                                bp_flag[bpc_sxlat[var->fun?
                                        var->index : bp_me_xlat2[var->index]
                                ]] = BP_EQUIV;
		...
		pds_insert_trans(&mc_pds,bp_qstate,label1,bp_qstate,label2,
			tmplabel, tmp2, ref? "*":"call",bp_prio(target->stmt));
		...
		Cudd_Ref(tmp2 = fullset);
		tmp2 = bpc_build_expr(fun,tmp2);
		pds_insert_trans(&mc_pds,targ1,tmplabel,
				targ2,0,0,tmp2, "*",bp_prio(stmt));
		Cudd_RecursiveDeref(mc_mgr,tmp2);
	}
						...
						Name label1, Name retlabel)
{
	...
	if (mc_verbose > 2) printf("special_eval %d\n",target->num_returns);
	while (base < target->num_returns)
	{
		...
		for (i = 0; i < stepsize; i++)
		{
			...
			Cudd_Ref(tmp1
				= Cudd_bddXnor(mc_mgr,bp_get_var(ref),
					ppvars[ret->index]));
		...
		pds_insert_trans(&mc_pds,bp_qstate,label1,
			bp_qstate,label2,0,tmp2,ref? "*" : NULL,bp_prio(stmt));
		Cudd_RecursiveDeref(mc_mgr,tmp2);
...
void bp_special_return (bp_fun_t fun, bp_stmt_t stmt, Name label1)
{
	DdNode *tmp1, *tmp2;
	Name targetstate,label2;
	bp_idref_t ref = stmt->e.a.asgnlist;
	...
	if (mc_verbose > 2) printf("special_return %d\n",fun->num_returns);
	while (base < fun->num_returns)
	{
		...
		for (i = 0; i < stepsize; i++)
		{
			...
			/* set a return value */
			tmp1 = bp_make_choose_expr(pvars[ret->index],ref->expr);
		...
		pds_insert_trans(&mc_pds,bp_qstate,label1,targetstate,label2,0,
			tmp2, ref? "*" : "return", 0);
		Cudd_RecursiveDeref(mc_mgr,tmp2);
						...
						Name label1, Name savelabel)
{
	...
	for (var = bp_globals; var; var = var->next)
	{
		...
		Cudd_Ref(tmp1 = Cudd_bddXnor(mc_mgr,
			ppvars[var->index],yvars[bp_savedglobals[var->index]]));
	...
	Cudd_Ref(tmp2 = fullset);
	tmp2 = bpc_build_expr(fun,tmp2);
	pds_insert_trans(&mc_pds,bp_qstate,label1,bp_qstate,savelabel,0,
			tmp2, "~", bp_prio(target->stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp2);
}

void bp_glob_restore (bp_fun_t fun, bp_stmt_t stmt, bp_fun_t target,
					  Name restorelabel, Name retlabel)
{
	DdNode *tmp1, *tmp2;
	bp_ident_t var;
	...
	for (var = bp_globals; var; var = var->next)
	{
		...
		Cudd_Ref(tmp1 = Cudd_bddXnor(mc_mgr,
			pvars[var->index],ypvars[bp_savedglobals[var->index]]));
	...
	pds_insert_trans(&mc_pds,bp_qstate,restorelabel,bp_qstate,retlabel,0,
			tmp2, "@", bp_prio(stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp2);
}
...
void bp_normal_call_save (bp_fun_t fun, bp_stmt_t stmt, bp_fun_t target,
					  Name savelabel, Name restorelabel)
{
	DdNode *tmp1, *tmp2;
	bp_idref_t ref;
	bp_ident_t var, parm;
	...
	{
		tmp1 = bp_make_choose_expr(yvars[parm->index],ref->expr);
	...
	for (var = bp_globals; var; var = var->next)
	{
		...
		Cudd_Ref(tmp1 = Cudd_bddXnor(mc_mgr,
		    ppvars[var->index],yppvars[bp_savedglobals[var->index]]));
						 ...
						 Name retlabel, Name label2)
{
	...
	pds_insert_trans(&mc_pds,
		bp_qstate,retlabel,bp_qstate,label2,0,tmp2,NULL,bp_prio(stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp2);
}
					...
					Name savelabel, Name restorelabel)
{
	...
	pds_insert_trans(&mc_pds,
		bp_qstate,savelabel,bp_qstate,
		    bp_create_label(target,target->stmt,0),restorelabel,
		tmp2, "call",bp_prio(target->stmt));
	Cudd_RecursiveDeref(mc_mgr,tmp2);
...
void bp_normal_return (bp_fun_t fun, bp_stmt_t stmt, Name label1)
{
	DdNode *tmp1, *tmp2;
	bp_idref_t ref;
	bp_ident_t parm;
	Name targetstate = mc_encoding? fun->funname : bp_qstate;

	bpc_clearflags(fun);
	for (ref = stmt->e.a.asgnlist, parm = fun->returns; ref; 
		ref = ref->next, parm = parm->next)
	{
		tmp1 = bp_make_choose_expr(pvars[parm->index],ref->expr);
	...
	pds_insert_trans(&mc_pds,
		bp_qstate,label1,targetstate,0,0,tmp2,"return",0);
	Cudd_RecursiveDeref(mc_mgr,tmp2);
}
