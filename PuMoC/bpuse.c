...
extern int mc_queue;
extern Name mc_reach_target_label;
...
bp_flow_t bp_mr_get_flownode (bp_fun_t fun, Name label)
{
		...
		switch (stmt->type)
		{
		  ...
		  case BP_GOTO:
			...
			if(stmt->e.a.flag==12341234)
			{
				...
				{
					bp_mr_insert_edge(fn1,stmt,bp_mr_get_flownode(fun,stmt->e.a.label2));
				}
			}
			break;
		  case BP_RETURN:
...
{
	bp_fun_t fun;
	common_resource_setmark(3);
	for (fun = bp_functions; fun; fun = fun->next)
		bp_mr_fun(fun);
	common_resource_displaymark(3,"live vars",NULL);
}
...
void bp_build_flow_graph ()
{
	...
	common_resource_displaymark(3,"flow graph",NULL);
}
...
void bp_build_call_graph ()
{
	...
	for (fun = bp_functions; fun; fun = fun->next)
	{
		...
		for (var = fun->locals, j = 0, cb = fun->callbuf; var; 
			...
			if (bpg_flag[var->index])
			{
				...
				cb->varname = name_create(tmp);
				cb->index = bp_assign_cb_index(cb->varname,bp_funcount,j++);
				if (mc_verbose)
					printf("adding %s for %s in %s\n",name_cstring(cb->varname),
						name_cstring(var->varname),name_cstring(fun->funname));
			}
	}
	...
	common_resource_displaymark(3,"globals",NULL);
}
...
void bp_compute_distances ()
{
	common_resource_setmark(3);
	if (!bp_flow_root)
		common_error("target label not found!");
	...
	common_resource_displaymark(3,"distances",NULL);
}
