...
typedef struct ParamListStruct * ParamList;
typedef struct ValueListStruct * ValList;
typedef int Name;
...
{
	int name;
	int type;            // 0=variable, 1= constant
	ParamList next;
};
...
void name_bal_delete (NameBalTree);
...
ParamList createParam(Name,int, ParamList);
int compareParams(ParamList,ParamList);
void deleteParams(ParamList);
ParamList paramsCopy(ParamList);

int findVarIndex(ParamList,Name);
int findValIndex(ValList,Name);

ValList createVal(Name, ValList);
int compareVal(ValList,ValList);
void deleteVal(ValList);

DdNode* build_conj_bdd(ParamList,ValList);
DdNode* build_disj_bdd(ParamList,ValList);
DdNode* build_var_bdd(Name,Name);
DdNode* varExistAbstract(DdNode* , Name);
